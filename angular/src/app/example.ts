...
// import * as Examples from 'shared-example'
// console.log(Object.keys(Examples));

function oneByOneMultiStyleGenerator(
  items: DataModel.Data[],
  styleIds: { style1: string; style2: string },
): DataModel.Data[] {
  if (typeof items === 'object' && Array.isArray(items)) {
    const result: DataModel.Data[] = [];
    items.forEach((item) => {
      const multiStyle: DataModel.MapMultiStyleValue = {};
      Object.entries(item).forEach(([key, value]) => {
        if (typeof value !== 'object') {
          multiStyle[key] = value
            .split('')
            .map((character: string, index: number) => {
              const styleId =
                index % 2 == 1 ? styleIds.style1 : styleIds.style2;
              return { value: character, styleId };
            });
        }
      });
      result.push({ ...item, multiStyleValue: multiStyle });
    });
    console.log(result);
...
function oneByOneHeaderMultiStyleGenerator(
  headers: DataModel.Header[],
  styleIds: { style1: string; style2: string },
): DataModel.Header[] {
  if (typeof headers === 'object' && Array.isArray(headers)) {
    const result: DataModel.Header[] = [];
    headers.forEach((header) => {
      let multiStyle: DataModel.MultiStyleValue[] = [];
      if (typeof header.text === 'string') {
        multiStyle = header.text
          .split('')
          .map((character: string, index: number) => {
            const styleId = index % 2 == 1 ? styleIds.style1 : styleIds.style2;
            return { value: character, styleId };
          });
      }
      result.push({ ...header, multiStyleValue: multiStyle });
    });
    console.log(result);
...
export function ex12(): DataModel.ExcelTable {
  const colorPalette = {
    c4: '61677A',
    c2: 'FFF6E0',
  };
...
export function ex13(): DataModel.ExcelTable {
  const colorPalette = {
    c4: 'F7E987',
    c2: '5B9A8B',
  };
...
export function ex14(): DataModel.ExcelTable {
  ...
  Header should start with ${d.toUpperCase()}`,
        ...
        headers: oneByOneHeaderMultiStyleGenerator(
          [
            {
              label: 'c1',
              text: '**',
        ...
        data: oneByOneMultiStyleGenerator(
          [
            { c1: '0.756', c2: '150', c3: 'John', c5: '1' },
            {
              c1: '0.238',
